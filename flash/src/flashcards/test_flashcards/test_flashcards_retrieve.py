"""
FlashCourses- Test cases for API endpoints for get and retrieve
for card and deck endpoints
Created By: Swechchha Tiwari  4/6/2018

Relative File Path:
/flash/src/flashcards/test_flashcards/test_flashcards_retrieve.py
...
class APIgetStatusCodeDeckCard(APITestCase):
    ...
    def setUp(self):
        """
        Sets up testing environment for GET method for card_retrieve
        and deck_retrieve based on unique_id.
        """
        ...
        card_tbl = Card.objects.create(
            front='test', back='testsback')

        self.retrieve_method_endpoints = [
            reverse(
                'flashcards:flashcards_api:deck_retrieve',
                kwargs={'unique_id': Deck.objects.first().unique_id}),

            reverse(
                'flashcards:flashcards_api:card_retrieve',
                kwargs={'unique_id': Card.objects.first().unique_id}),

        ]

    def test_flashcards_endpoint_retrieve_method(self):
        """
        Create a request to every endpoint in retrieve_method_endpoints.
